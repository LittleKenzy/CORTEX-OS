...
class PatternDetector {
    ...
    async analyzeHabit(habitId) {
        ...
        if (afterSuccessPattern)
            patterns.push(afterSuccessPattern);
        // Detect after-failure cascades
        const afterFailurePattern = this.detectAfterFailurePattern(entries);
        if (afterFailurePattern)
            patterns.push(afterFailurePattern);
    ...
    detectDayOfWeekPattern(entries) {
        ...
        if (worstRate < 0.6)
            return null; // Must fail at least 60% of the time
    ...
    detectAfterSuccessPattern(entries) {
    ...
    detectAfterFailurePattern(entries) {
        let cascadeCount = 0;
