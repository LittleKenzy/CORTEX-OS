...
import { PriorityScore, PriorityFactors } from '@cortex/shared/types';
...
export class PriorityScorer {
  ...
  async calculatePriority(taskId: string): Promise<PriorityScore> {
  ...
  private calculateUrgency(dueDate: Date | null, now: Date): number {
    if (!dueDate) return 0.3; // Default medium urgency
    
    const daysUntilDue = (dueDate.getTime() - now.getTime()) / (1000 * 60 * 60 * 24);
    
    if (daysUntilDue < 0) return 1.0; // Overdue
    if (daysUntilDue < 1) return 0.95; // Due today
    if (daysUntilDue < 3) return 0.85; // Due within 3 days
    if (daysUntilDue < 7) return 0.7; // Due this week
    if (daysUntilDue < 30) return 0.5; // Due this month
    
    return 0.3; // Due later
  }

  private calculateEffort(estimatedMinutes: number | null): number {
    if (!estimatedMinutes) return 0.5;
    
    // Quick tasks (< 30 min) score higher
    // Longer tasks score lower
    if (estimatedMinutes <= 30) return 1.0;
    if (estimatedMinutes <= 60) return 0.8;
    if (estimatedMinutes <= 120) return 0.6;
    if (estimatedMinutes <= 240) return 0.4;
    
    return 0.2;
  }

  private async calculateImportance(taskId: string): Promise<number> {
    ...
    if (blockingCount === 0) return 0.5;
    if (blockingCount <= 2) return 0.7;
    if (blockingCount <= 5) return 0.85;
    
    return 1.0;
  }

  private async calculateDependencies(taskId: string): Promise<number> {
    ...
    if (childCount <= 7) return 0.8;
    
    return 1.0;
  }

  private calculateAge(createdAt: Date, now: Date): number {
    const daysOld = (now.getTime() - createdAt.getTime()) / (1000 * 60 * 60 * 24);
    
    if (daysOld < 1) return 0.1;
