import { BiasAnalysisReport, CognitiveBias } from './../../../../shared/src/types/decision.types';
import Database from "better-sqlite3";

export class BiasAnalyzer {
  constructor(private db: Database.Database) {}

  async analyzeDecision(decisionId: string): Promise<CognitiveBias[]> {
  ...
  private detectConfirmationBias(decision: any): CognitiveBias | null {
    const reasoning = decision.reasoning.toLowerCase();
    const indicators: string[] = [];

    // Check for one-sided reasoning
    const prosCount = (reasoning.match(/pro|benefit|advantage|positive/g) || [])
      .length;
    const consCount = (
      reasoning.match(/con|drawback|disadvantage|negative|risk/g) || []
    ).length;
  ...
  private detectOptimismBias(decision: any): CognitiveBias | null {
    if (!decision.expectedOutcome) return null;
  ...
  private detectAnchoring(decision: any): CognitiveBias | null {
  ...
  private detectSunkCost(decision: any): CognitiveBias | null {
    const context = decision.context.toLowerCase();
    const reasoning = decision.reasoning.toLowerCase();
    const indicators: string[] = [];
  ...
  async generateReport(
    ...
    for (const decision of decisions) {
      ...
      if (decision.actualOutcome) {
        ...
        if (similarity > 0.7) accurateOutcomes++;
      }
    ...
    const mostCommonBiases = Array.from(biasFrequency.entries())
      .map(([type, occurrences]) => ({
        type: type as CognitiveBias["type"],
        occurrences,
    ...
    return {
      ...
      mostCommonBiases,
      accuracyRate:
        totalWithOutcome > 0 ? (accurateOutcomes / totalWithOutcome) * 100 : 0,
      averageConfidence:
  ...
  private calculateOutcomeSimilarity(expected: string, actual: string): number {
  ...
  private generateRecommendations(biases: any[]): string[] {
    ...
    if (biases.find((b) => b.type === "optimism_bias")) {
      recommendations.push(
        "List potential risks and worst-case scenarios explicitly"
      );
    }
